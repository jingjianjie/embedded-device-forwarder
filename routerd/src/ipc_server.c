...
void process_frame(uint8_t* buf,int n){
     ...
     if (n < sizeof(ipc_header_t)) {
        printf("[IPC] frame too small: %d bytes\n", n);
        return;
    }
    ...
    if (sizeof(ipc_header_t) + payload_len > n) {
        printf("[IPC] incomplete frame: need %u bytes, got %d\n",
...
void ipc_handle_frame(uint16_t cmd, uint8_t* payload, uint32_t len)
{
    ...
    case CMD_SET_ROUTES: {
        if (len < sizeof(int)) {
            printf("[IPC] SET_ROUTES payload too small\n");
            return;
        }
        ...
        if (len < expect) {
            printf("[IPC] SET_ROUTES payload mismatch\n");
            return;
        }
